...
每次采集会先重载已经打开的额度管理标签页，等待页面加载完成以及“当前时段/近 7 天”额度区出现，再提取数字。这样不会把长期未刷新的旧 DOM 当作刚采集的数据。

套餐名称只接受“订阅与额度管理”标题紧邻的套餐字段；账号昵称等页面文本不会作为套餐名称缓存或发布。百分比必须位于 0–100，重置时间也必须是有效日期和时间，否则整次同步失败并回退到已有缓存。

它不进入豆包客户端进程，不读取 Cookie、localStorage、登录 Token 或 Chrome 配置，也不新开标签页或点击页面。唯一的页面操作是重载用户已经打开的固定额度页。原始 DOM 不落盘；缓存文件只含归一化字段，位于：

`~/.cache/ai-quota-monitor/doubao-quota.json`

文件权限固定为 `0600`。实时刷新失败时，仪表盘卡片标“使用缓存”，备注只保留采集时间和短原因（例如“未开额度页”），并停止用这份旧数据产生刷新提醒；超过 6 小时还会注明过期时长。可通过 `DOUBAO_QUOTA_MAX_AGE_HOURS` 调整时间阈值。
...
成功后会输出已归一化的 JSON，并更新本地缓存。普通的 `python3 quota_report.py`、`--json`、`--html` 和 `publish.sh` 都会把豆包作为一个额度源；采集时会先刷新标签页再读取 DOM，失败则使用缓存并明确标为非实时。

刷新额度页只是普通网页请求，不会提交提示词或发起模型生成，因此不消耗豆包对话/生成 token。
...
- `豆包额度页刷新后未加载`：检查网络或登录状态；本次会显示旧缓存而不是冒充实时值。
